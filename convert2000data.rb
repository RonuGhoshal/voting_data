...
csv_data.each do |row|
  row[:other] = (100 - (row[:bush].to_f + row[:gore].to_f)).to_s + "%"
  if row[:other].to_i == 100
    row[:other] = 0
  end
  rgb = [row[:bush], row[:other], row[:gore]].map do |candidate|
...
CSV.open("new_modified2000data.csv", "wb") do |csv|
  csv << ["id", "margin", "bush", "gore", "trash1", "trash2", "other", "hex_color"]
  csv_data.each do |row|
    csv << row.values
  end
end