...
csv_data.each do |row|
  ...
  rgb = [row[:harding], row[:other], row[:cox]].map do |candidate|
      if candidate == "-"
        0
      else
...
p csv_data[0]
CSV.open("modified1920data.csv", "wb") do |csv|
  csv << ["id", "margin", "harding","cox", "other", "other2", "hex_color"]
  csv_data.each do |row|
    csv << row.values
  end
end