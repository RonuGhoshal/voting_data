...
csv_data.each do |row|
  rgb = [row[:romney], row[:other], row[:obama]].map do |candidate|
...
CSV.open("new_modified2012data.csv", "wb") do |csv|
  csv << ["id", "margin", "romney", "obama", "other", "hex_color"]
  csv_data.each do |row|
    csv << row.values
  end
end