...
csv_data.each do |row|
  rgb = [row[:hoover], row[:other], row[:roosevelt]].map do |candidate|
      if candidate == "-"
        0
      else
...
CSV.open("modified1932data.csv", "wb") do |csv|
  csv << ["id", "margin", "hoover", "roosevelt", "other", "hex_color"]
  csv_data.each do |row|
    csv << row.values
  end
end