...
csv_data.each do |row|
  row[:other] = (row[:other].to_f + row[:hall].to_f).to_s + "%"
  rgb = [row[:reagan], row[:other], row[:mondale]].map do |candidate|
      if candidate == "-"
        0
      else
...
CSV.open("modified1984data.csv", "wb") do |csv|
  csv << ["id", "margin", "reagan", "mondale", "hall", "other", "hex_color"]
  csv_data.each do |row|
    csv << row.values
  end
end