...
csv_data.each do |row|
  row[:other] = (100 - (row[:bush].to_f + row[:kerry].to_f)).to_s + "%"
  if row[:other].to_i == 100
    row[:other] = 0
  end
  rgb = [row[:bush], row[:other], row[:kerry]].map do |candidate|
...
CSV.open("new_modified2004data.csv", "wb") do |csv|
  csv << ["id", "margin", "bush", "kerry", "trash1", "trash2", "trash1", "other", "hex_color"]
  csv_data.each do |row|
    csv << row.values
  end
end