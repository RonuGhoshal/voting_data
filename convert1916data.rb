...
csv_data.each do |row|
  ...
  rgb = [row[:hughes], row[:other], row[:wilson]].map do |candidate|
      if candidate == "-"
        0
      else
...
p csv_data[0]
CSV.open("modified1916data.csv", "wb") do |csv|
  csv << ["id", "margin", "hughes","wilson", "other", "other2", "hex_color"]
  csv_data.each do |row|
    csv << row.values
  end
end