...
csv_data.each do |row|
  rgb = [row[:goldwater], row[:other], row[:johnson]].map do |candidate|
...
CSV.open("new_modified1964data.csv", "wb") do |csv|
  csv << ["id", "margin", "goldwater", "johnson", "other", "hex_color"]
  csv_data.each do |row|
    csv << row.values
  end
end