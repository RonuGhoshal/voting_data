...
csv_data.each do |row|
  rgb = [row[:ford], row[:other], row[:carter]].map do |candidate|
...
CSV.open("modified1976data.csv", "wb") do |csv|
  csv << ["id", "margin", "ford", "carter", "other", "hex_color"]
  csv_data.each do |row|
    csv << row.values
  end
end